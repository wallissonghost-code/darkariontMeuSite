...
const routeUrl=route=>`${routes[route]}?embedded=1&tabActive=1&v=${BUILD}`;

function aplicarTemaNoFrame(frame,theme=temaAtual()){
  if(!frame)return;
  try{const doc=frame.contentDocument;if(doc){doc.documentElement.dataset.theme=theme;doc.documentElement.style.colorScheme=theme}frame.contentWindow?.postMessage({type:'wd-theme-change',theme},location.origin)}catch(error){console.warn('Não foi possível sincronizar o tema da tela:',error)}
}
function sincronizarTema(theme){if(!temaValido(theme))return;document.documentElement.dataset.theme=theme;document.documentElement.style.colorScheme=theme;localStorage.setItem('wd-theme',theme);aplicarTemaNoFrame(currentFrame,theme)}
...
function prepararFrame(frame){
  try{
    ...
    doc.documentElement.dataset.embedded='true';doc.documentElement.dataset.tabActive='true';
...
function suspenderFrame(frame){
  if(!frame)return;
  try{frame.contentWindow?.postMessage({type:'wd-tab-lifecycle',active:false},location.origin);frame.contentWindow?.AppFirebase?.setActive?.(false)}catch(error){console.warn('Não foi possível suspender a tela anterior:',error)}
}
function encerrarFrame(frame){
  if(!frame)return;
  suspenderFrame(frame);
  try{frame.contentWindow?.AppFirebase?.destroy?.()}catch(error){console.warn('Não foi possível destruir o ciclo anterior:',error)}
...
function createFrame(route,token,previousFrame){
  const frame=document.createElement('iframe');
  frame.className='app-view';frame.title=route;frame.dataset.route=route;frame.setAttribute('aria-hidden','true');frame.style.visibility='hidden';frame.style.pointerEvents='none';frame.src=routeUrl(route);
  frame.addEventListener('load',()=>{
    if(token!==navigationToken){encerrarFrame(frame);return}
    prepararFrame(frame);
    requestAnimationFrame(()=>{
      if(token!==navigationToken){encerrarFrame(frame);return}
      frame.classList.add('is-active');frame.setAttribute('aria-hidden','false');frame.style.visibility='visible';frame.style.pointerEvents='auto';
      currentFrame=frame;
      if(previousFrame&&previousFrame!==frame)encerrarFrame(previousFrame);
...
function showRoute(route,{push=true}={}){
  route=normalizeRoute(route);if(route===currentRoute&&currentFrame)return;
  const token=++navigationToken,previousFrame=currentFrame;
  currentRoute=route;updateNav(route);

  // A tela anterior para de ouvir o Firebase imediatamente, mas permanece
  // visualmente congelada até a próxima terminar de carregar. Isso elimina
  // tanto conexões simultâneas quanto o flash branco entre rotas.
  suspenderFrame(previousFrame);
  createFrame(route,token,previousFrame);

  if(!firstPaintDone)loader.classList.remove('is-hidden');else loader.classList.add('is-hidden');
