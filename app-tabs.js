...
function cleanEmbeddedFrame(frame){
  try{
    ...
    const hide=()=>{
      doc.querySelectorAll('.menu,.mobile-menu-trigger,.menu-backdrop,.client-bottom-nav,.app-bottom-nav').forEach(el=>el.remove());
...
function ensureFrame(route){
  ...
  frame.title=route;
  frame.loading=route==='home'?'eager':'lazy';
  frame.src=routes[route];
  frame.dataset.route=route;
  frame.addEventListener('load',()=>{
    cleanEmbeddedFrame(frame);
    frame.dataset.ready='true';
    if(route===currentRoute)loader.classList.add('is-hidden');
  });
  stage.append(frame);
...
function updateNav(route){
  nav.querySelectorAll('[data-route]').forEach(link=>link.classList.toggle('is-active',link.dataset.route===route));
}
...
function showRoute(route,{push=true}={}){
  ...
  loader.classList.toggle('is-hidden',frame.dataset.ready==='true');
  requestAnimationFrame(()=>frames.forEach((item,key)=>item.classList.toggle('is-active',key===route)));
...
if(document.querySelector('.menu'))menuObserver.observe(document.querySelector('.menu'),{childList:true,subtree:true,attributes:true,attributeFilter:['class','style']});
limparMenuExterno();
showRoute(routeFromHash(),{push:false});
const preload=()=>Object.keys(routes).forEach((route,index)=>setTimeout(()=>ensureFrame(route),index*100));
if('requestIdleCallback'in window)requestIdleCallback(preload,{timeout:800});else setTimeout(preload,350);
