...
function publish(next) {
  ...
  subscribers.forEach(listener => {
    try { listener(current); } catch (error) { console.error('Erro em assinante da sessão:', error); }
  });
  document.dispatchEvent(new CustomEvent('wd-session-change', { detail: current }));
  if (!readySettled && next.status !== 'loading') {
    readySettled = true;
...
function resetClientState() {
  ...
  Object.keys(localStorage).forEach(key => {
    if (key.startsWith('wd-user:') || key.startsWith('wd-profile:') || key.startsWith('currentUser_')) {
...
window.WDSession = {
  ...
  get isReady() { return current.status === 'ready'; }
};
