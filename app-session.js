...
const SIGNED_OUT_GRACE_MS=4500;
const PROFILE_CONFIRM_MS=2500;
...
function publishSignedOutWithGrace(){
  cancelSignedOut();
  const hint=readSessionHint();
  if(!hint){publish({status:'signed-out',user:null,profile:null,error:null});return}
  publish({status:'restoring',user:null,profile:null,error:null});
  signedOutTimer=setTimeout(()=>{
    signedOutTimer=0;
    if(auth.currentUser)return;
    clearSessionHint();
    publish({status:'signed-out',user:null,profile:null,error:null});
  },SIGNED_OUT_GRACE_MS);
}

async function confirmMissingProfile(user,cachedProfile){
  clearTimeout(profileMissingTimer);
  profileMissingTimer=setTimeout(async()=>{
    try{
...
document.addEventListener('visibilitychange',()=>{if(!document.hidden&&current.status==='restoring'&&auth.currentUser){cancelSignedOut();saveSessionHint(auth.currentUser)}});
