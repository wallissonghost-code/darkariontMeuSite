...
function montarMenuAdmin(){
  ...
  aside.innerHTML=`<div class="wd-admin-brand"><strong>WD</strong><span>FOUNDER ADMIN</span></div><nav class="wd-admin-links"><a href="app.html?page=home" class="wd-admin-member-return">← Voltar para área do membro</a>${ADMIN_TOOLS.map(([href,label])=>`<a href="${href}" class="${current===href?'is-active':''}" ${current===href?'aria-current="page"':''}>${label}</a>`).join('')}</nav><div class="wd-admin-footer"><a href="app.html?page=home">Área do membro</a><button type="button" data-action="logout">Sair da conta</button></div>`;
  const trigger=document.createElement('button');trigger.className='wd-admin-mobile-trigger';trigger.type='button';trigger.setAttribute('aria-label','Abrir menu administrativo');trigger.textContent='☰';
  const backdrop=document.createElement('button');backdrop.className='wd-admin-backdrop';backdrop.type='button';backdrop.setAttribute('aria-label','Fechar menu administrativo');
  trigger.addEventListener('click',()=>body.classList.toggle('wd-admin-menu-open'));backdrop.addEventListener('click',fecharAdminMenu);aside.addEventListener('click',event=>{if(event.target.closest('a'))fecharAdminMenu()});
  document.body.prepend(aside);document.body.append(trigger,backdrop);document.querySelector('.menu')?.setAttribute('hidden','');
}
...
function montarAcessosAdminSpa(){
  ...
  const desktopNav=document.querySelector('.spa-sidebar .spa-nav'),bottomNav=document.querySelector('.spa-bottom-nav');if(!desktopNav||!bottomNav)return;
  const desktopSection=document.createElement('section');desktopSection.className='spa-admin-section';desktopSection.dataset.spaAdminTools='true';desktopSection.innerHTML=`<div class="spa-admin-section-title"><span>ADMINISTRAÇÃO</span><b>Acesso autorizado</b></div><div class="spa-admin-links">${ADMIN_TOOLS.map(([href,label,,icon])=>`<a href="${href}"><span>${icon}</span><small>${label}</small></a>`).join('')}</div>`;desktopNav.insertAdjacentElement('afterend',desktopSection);
  const mobileTrigger=document.createElement('button');mobileTrigger.type='button';mobileTrigger.className='spa-admin-nav-button';mobileTrigger.dataset.adminToolsTrigger='true';mobileTrigger.setAttribute('aria-label','Abrir ferramentas administrativas');mobileTrigger.setAttribute('aria-expanded','false');mobileTrigger.innerHTML='<span>✦</span><small>Admin</small>';mobileTrigger.addEventListener('click',alternarFerramentasAdmin);bottomNav.append(mobileTrigger);bottomNav.classList.add('has-admin');
  const panel=document.createElement('section');panel.className='spa-admin-tools-panel';panel.dataset.spaAdminTools='true';panel.setAttribute('aria-label','Ferramentas administrativas');panel.innerHTML=`<div class="spa-admin-tools-head"><div><span>WD FOUNDER</span><h2>Ferramentas administrativas</h2><p>Use as ferramentas e volte para a área do membro sem encerrar sua sessão.</p></div><button type="button" data-close-admin-tools aria-label="Fechar">×</button></div><div class="spa-admin-tools-grid">${ADMIN_TOOLS.map(([href,label,description,icon])=>`<a href="${href}"><span class="spa-admin-tool-icon">${icon}</span><div><strong>${label}</strong><small>${description}</small></div><b>›</b></a>`).join('')}</div>`;
  const backdrop=document.createElement('button');backdrop.type='button';backdrop.className='spa-admin-tools-backdrop';backdrop.dataset.spaAdminTools='true';backdrop.setAttribute('aria-label','Fechar ferramentas administrativas');panel.querySelector('[data-close-admin-tools]').addEventListener('click',fecharFerramentasAdmin);backdrop.addEventListener('click',fecharFerramentasAdmin);panel.addEventListener('click',event=>{if(event.target.closest('a'))fecharFerramentasAdmin()});document.body.append(panel,backdrop);document.addEventListener('keydown',event=>{if(event.key==='Escape')fecharFerramentasAdmin()});
}
function removerAcessosAdminSpa(){document.querySelectorAll('[data-spa-admin-tools],[data-admin-tools-trigger]').forEach(element=>element.remove());document.querySelector('.spa-bottom-nav')?.classList.remove('has-admin');fecharFerramentasAdmin()}

async function sair(event){event?.preventDefault?.();event?.stopPropagation?.();if(saindo)return;saindo=true;document.querySelectorAll('[data-action="logout"],.logout-btn').forEach(button=>{button.disabled=true;button.textContent='Saindo...'});try{window.WDSession?.resetClientState?.();await signOut(auth)}catch(error){console.error('Erro ao sair:',error)}finally{location.replace('index.html?logout=1')}}
...
function renderReady(state){
  ...
  aplicarTema(tema,user.uid);document.documentElement.dataset.role=admin?'admin':'cliente';
  if(body.dataset.adminPage==='true'&&!admin){location.replace('app.html?page=home');return}
  if(body.dataset.adminPage==='true')montarMenuAdmin();else montarMenuCliente(profile);
  if(isSpaShell){const avatar=iniciais(profile?.nome||user.displayName||'WD');document.querySelectorAll('[data-spa-avatar]').forEach(element=>element.textContent=avatar);if(admin)montarAcessosAdminSpa();else removerAcessosAdminSpa()}
  body.classList.add('wd-auth-ready');document.documentElement.dataset.authState='ready';document.dispatchEvent(new CustomEvent('wd-role-ready',{detail:{user,role,admin,dados:profile}}));
}
