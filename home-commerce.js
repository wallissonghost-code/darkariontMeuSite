...
function render(){
  ...
  cartCount.textContent=String(state.waiting.length);
  notifyCount.textContent=String(unread);
  notifyCount.hidden=unread===0;
  cartList.innerHTML=state.waiting.length?state.waiting.map(item=>{
...
cartButton?.addEventListener('click',()=>{const open=cartPanel.hidden;closePanels();cartPanel.hidden=!open});
notifyButton?.addEventListener('click',async()=>{const open=notifyPanel.hidden;closePanels();notifyPanel.hidden=!open;if(open)await markNotificationsRead()});
...
initMemberCommerce().then(render).catch(error=>{console.error('Falha ao carregar carrinho e notificações:',error);cartList.innerHTML='<div class="commerce-empty">Não foi possível carregar agora.</div>'});
