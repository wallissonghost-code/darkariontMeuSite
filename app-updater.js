...
async function registerWorker(){if(!('serviceWorker' in navigator))return null;const registration=await navigator.serviceWorker.register(`./sw.js?v=${LOCAL_VERSION}`,{scope:'./',updateViaCache:'none'});await registration.update().catch(()=>{});return registration}
...
(async()=>{try{await clearLegacyCaches();await registerWorker()}catch(error){console.warn('Service Worker não pôde ser ativado:',error)}await checkForUpdate();setInterval(checkForUpdate,CHECK_INTERVAL);window.addEventListener('online',checkForUpdate)})();
