...
function loadRuntimeScript(src,id){
  ...
  script.id=id;script.src=`${src}?v=${LOCAL_VERSION}`;script.defer=true;
  document.head.append(script);
}
function loadRuntimeStyle(href,id){
  if(document.getElementById(id))return;
  const link=document.createElement('link');
  link.id=id;link.rel='stylesheet';link.href=`${href}?v=${LOCAL_VERSION}`;
  document.head.append(link);
}

// Estilos essenciais entram imediatamente. Nenhum controlador extra de navegação é carregado.
...
async function registerWorker(){
  ...
  return navigator.serviceWorker.register(`./sw.js?v=${LOCAL_VERSION}`,{scope:'./',updateViaCache:'none'});
}
const idle=callback=>'requestIdleCallback'in window?requestIdleCallback(callback,{timeout:2500}):setTimeout(callback,1200);

requestAnimationFrame(()=>requestAnimationFrame(()=>{
  // Runtime móvel não controla mais a rota; a SPA é a única fonte do estado ativo.
  ...
  setTimeout(()=>{
    registerWorker().then(registration=>registration?.update().catch(()=>{})).catch(()=>{});
...
window.addEventListener('online',()=>setTimeout(checkForUpdate,1200),{passive:true});
