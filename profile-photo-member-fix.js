...
async function compressPhoto(file){if(!file?.type?.startsWith('image/'))throw new Error('Selecione uma imagem válida.');if(file.size>MAX_FILE_SIZE)throw new Error('A foto deve ter no máximo 10 MB.');const image=await timeout(loadImage(file),12000,'A imagem demorou demais para abrir.');const width=image.naturalWidth||image.width,height=image.naturalHeight||image.height,side=Math.min(width,height);if(!side)throw new Error('A imagem não possui dimensões válidas.');const canvas=document.createElement('canvas');canvas.width=PHOTO_SIZE;canvas.height=PHOTO_SIZE;const context=canvas.getContext('2d',{alpha:false});context.fillStyle='#111';context.fillRect(0,0,PHOTO_SIZE,PHOTO_SIZE);context.drawImage(image,(width-side)/2,(height-side)/2,side,side,0,0,PHOTO_SIZE,PHOTO_SIZE);const blob=await new Promise((resolve,reject)=>canvas.toBlob(result=>result?resolve(result):reject(new Error('Não foi possível processar a imagem.')),'image/jpeg',.82));const preview=canvas.toDataURL('image/jpeg',.72);return{blob,preview}}
async function uploadPhoto(user,processed){const path=ref(storage,`usuarios/${user.uid}/perfil.jpg`);await timeout(uploadBytes(path,processed.blob,{contentType:'image/jpeg',cacheControl:'public,max-age=3600'}),25000,'O envio da foto demorou demais.');return timeout(getDownloadURL(path),12000,'Não foi possível obter a foto enviada.')}
...
async function install(){
  ...
  input.addEventListener('change',async()=>{
    ...
    try{
      ...
      localStorage.setItem(localKey,processed.preview);
      showPhoto(processed.preview,name,{keepCurrent:true});
      message('Enviando sua foto...');
      const photo=await uploadPhoto(user,processed);
      await timeout(updateProfile(auth.currentUser||user,{photoURL:photo}),12000,'A foto foi enviada, mas o perfil não pôde ser atualizado.');
      localStorage.setItem(localKey,photo);
      showPhoto(photo,name,{keepCurrent:true});
      try{await timeout(setDoc(doc(db,'usuarios',user.uid),{foto:photo,atualizadoEm:serverTimestamp()},{merge:true}),10000,'Firestore indisponível')}catch(error){console.warn('Foto salva no Auth/Storage; sincronização do documento aguardando regra do Firebase:',error)}
      if(window.WDSession?.state?.profile)window.WDSession.state.profile.foto=photo;
      document.dispatchEvent(new CustomEvent('wd-profile-photo-updated',{detail:{foto:photo,uid:user.uid}}));
      message('Foto de perfil atualizada com sucesso.');
    }catch(error){
      console.error('Falha ao salvar foto:',error);
...
const activate=()=>install().catch(error=>console.error('Falha ao iniciar foto universal:',error));
activate();
