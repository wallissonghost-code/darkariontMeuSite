...
function showPhoto(url,name){const image=document.getElementById('profileAvatarImage'),fallback=document.getElementById('profileAvatarInitials');if(!image||!fallback)return;fallback.textContent=initials(name);if(!url){image.hidden=true;image.removeAttribute('src');fallback.hidden=false;return}const probe=new Image();probe.onload=()=>{image.src=url;image.hidden=false;fallback.hidden=true};probe.onerror=()=>{image.hidden=true;fallback.hidden=false};probe.src=url}
...
async function install(){
  ...
  input.addEventListener('change',async()=>{
    ...
    try{
      ...
      const previous=String(window.WDSession?.state?.profile?.foto||localPhoto||user.photoURL||'').trim();showPhoto(previous,name);message(error.message||'Não foi possível atualizar a foto.',true);
