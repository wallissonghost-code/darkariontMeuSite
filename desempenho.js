...
function spendLabel(total){if(total>=18000)return 'CLIENTE ELITE';if(total>=6000)return 'ALTO VALOR';if(total>=1500)return 'RECORRENTE';if(total>0)return 'EM DESENVOLVIMENTO';return 'SEM COMPRAS'}
...
async function load(){
  ...
  members=users.map(u=>{
    ...
    const total=own.reduce((s,p)=>s+Math.max(0,Number(p.valor)||0),0);
    const calc=calcularFidelidade(total),last=own[0]?.date||null;
    return {...u,purchases:own,totalSpent:total,lastPurchase:last,purchaseCount:own.length,activity:classify(last),calculatedVip:calc.vip,calculatedStamps:calc.carimbos,isDifferent:(Number(u.vip)||0)!==calc.vip||(Number(u.carimbos)||0)!==calc.carimbos};
  });
...
function renderMembers(){
  ...
  document.getElementById('membersList').innerHTML=list.length?list.map(m=>`<article class="member-card"><div class="member-top"><div><h3>${m.nome||'Cliente sem nome'}</h3><p>${m.email||'Sem e-mail'}${m.telefone?` · ${m.telefone}`:''}</p></div><span class="status-badge status-${m.activity.key}">${m.activity.label}</span></div>${m.isDifferent?'<div class="sync-warning">O nível salvo está diferente do histórico. Nenhuma alteração automática foi feita.</div>':''}<div class="member-meta"><div><span>Nível pelo histórico</span><strong>LV${m.calculatedVip} ${levels[m.calculatedVip]}</strong></div><div><span>Total gasto</span><strong>${money(m.totalSpent)}</strong></div><div><span>Compras</span><strong>${m.purchaseCount}</strong></div><div><span>Última compra</span><strong>${dateLabel(m.lastPurchase)}</strong></div></div><div class="member-meta"><div><span>Perfil</span><strong>${spendLabel(m.totalSpent)}</strong></div><div><span>Ticket médio</span><strong>${money(m.purchaseCount?m.totalSpent/m.purchaseCount:0)}</strong></div><div><span>Carimbos</span><strong>${m.calculatedStamps}/10</strong></div><div><span>Bônus disponível</span><strong>${money(m.creditos)}</strong></div></div><div class="member-actions"><a class="open-admin" href="perfil-admin.html?cliente=${encodeURIComponent(m.id)}">Corrigir conta</a><a class="open-delete" href="excluir-cliente.html?cliente=${encodeURIComponent(m.id)}">Excluir conta</a></div></article>`).join(''):'<div class="loading-state">Nenhum membro encontrado com esses filtros.</div>';
}
