...
function cardTemplate(product) {
  ...
  return `<button class="product-card" type="button" data-product-id="${esc(product.id)}" data-priority="${priorityOf(product)}" data-stock="${stock}" aria-label="Abrir ${esc(product.nome || 'produto')}"><span class="product-media ${images[0] ? '' : 'no-image'}">${image}<span class="product-media-top">${product.selo ? `<span class="product-badge">${esc(product.selo)}</span>` : ''}</span>${images.length > 1 ? `<span class="photo-count">${images.length} fotos</span>` : ''}</span><span class="product-body"><span class="product-category">${esc(product.categoria || 'WD Founder')}</span><span class="product-title">${esc(product.nome || 'Mercadoria')}</span><span class="product-price-row"><span class="product-price">${oldPrice}<span class="product-price-line"><strong>${money(product.preco)}</strong>${discount ? `<span class="product-discount-inline">-${discount}%</span>` : ''}</span></span><span class="product-stock">Estoque: ${stock}</span></span></span></button>`;
}
