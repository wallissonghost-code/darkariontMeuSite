...
function duplicateCount(c){return clients.filter(x=>x.id!==c.id&&c.email&&normalized(x.email)===normalized(c.email)).length}
function optionLabel(c){const you=c.id===auth.currentUser?.uid?' · VOCÊ':'';const duplicate=duplicateCount(c)?' · POSSÍVEL DUPLICADO':'';const email=c.email||'sem e-mail';return `${c.nome||email} — LV${Math.max(1,(Number(c.vip)||0)+1)} · ${roleLabel(c.role)} · ${email} · ID ${shortUid(c.id)}${you}${duplicate}`}
