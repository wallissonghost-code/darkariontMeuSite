/* WD Founder — atualização forçada disponível apenas para administradores */
...
function refreshHref(){return '#forcar-atualizacao'}
function desktopItem(){return `<a href="${refreshHref()}" data-force-app-update><span class="wd-admin-icon-synced" data-icon-key="refresh">${REFRESH_ICON}</span><small>Forçar atualização</small></a>`}
function mobileItem(){return `<a href="${refreshHref()}" data-force-app-update><span class="spa-admin-tool-icon wd-admin-icon-synced" data-icon-key="refresh">${REFRESH_ICON}</span><div><strong>Forçar atualização</strong><small>Limpar cache e carregar a versão mais recente</small></div><b>›</b></a>`}

function installRefreshOption(root=document){
  root.querySelectorAll('.spa-admin-links').forEach(list=>{
    if(!list.querySelector('[data-force-app-update]'))list.insertAdjacentHTML('beforeend',desktopItem());
  });
  root.querySelectorAll('.spa-admin-tools-grid').forEach(list=>{
    if(!list.querySelector('[data-force-app-update]'))list.insertAdjacentHTML('beforeend',mobileItem());
  });
}
...
async function forceUpdate(button){
  ...
  if(!confirmed)return;
  refreshing=true;
  const originalTitle=button?.querySelector('strong,small')?.textContent||'';
  ...
  try{
    ...
    if(title)title.textContent=originalTitle||'Forçar atualização';
...
const observer=new MutationObserver(mutations=>{if(mutations.some(item=>item.addedNodes.length))installRefreshOption()});
