...
(async()=>{try{const session=await window.WDSession?.ready;currentUid=session?.user?.uid||window.WDSession?.user?.uid||'';currentProfile=session?.profile||session?.dados||{};adminAccess=ADMIN_ROLES.has(String(currentProfile.role||'').toLowerCase());try{const configSnap=await getDoc(doc(db,'configuracoes','rank'));if(configSnap.exists()){const data=configSnap.data();config={minimumSpend:Math.max(500,Number(data.minimumSpend)||500),weekly:Array.isArray(data.weekly)?data.weekly:DEFAULT_CONFIG.weekly,monthly:Array.isArray(data.monthly)?data.monthly:DEFAULT_CONFIG.monthly}}}catch(error){console.warn('Configuração do Rank indisponível:',error)}const canView=adminAccess||Math.max(0,Number(currentProfile.totalGasto)||0)>=config.minimumSpend;if(!canView){showLocked();return}showUnlocked();const usersSnapshot=await getDocs(collection(db,'usuarios'));users=usersSnapshot.docs.map(document=>({id:document.id,...document.data()}));try{const purchasesSnapshot=await getDocs(collection(db,'compras'));purchases=purchasesSnapshot.docs.map(document=>document.data())}catch(error){purchasesAvailable=false;console.warn('Compras do ciclo indisponíveis para esta conta:',error)}if(!purchasesAvailable){period='all';root.querySelectorAll('[data-rank-period]').forEach(button=>button.classList.toggle('is-active',button.dataset.rankPeriod==='all'));root.querySelectorAll('[data-rank-period]:not([data-rank-period="all"])').forEach(button=>button.disabled=true)}render();tick();timer=setInterval(tick,1000)}catch(error){console.error('Falha ao carregar ranking:',error);if(podium)podium.innerHTML='<div class="rank-loading">O ranking está temporariamente indisponível.</div>';if(listEl)listEl.innerHTML=''}})();
